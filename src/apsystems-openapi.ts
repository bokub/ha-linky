...
export class ApsOpenApi {
   ...
   */
  constructor(private appliId: string, private appliSecret: string) {
  ...
  async getEcuDailyConsumption(systemId: string, ecuId: string, monthStr: string): Promise<number[]> {
    ...
    const result = await this.callApi(
      `systems/${systemId}/devices/ecu/energy/${ecuId}`,
      {
        energy_level: 'daily',
        date_range: monthStr
      }
    ...
    return axios
      ...
      .then((res) => {
        if (res.data.code === 0 || res.data.code === 1000) {
          ...
          throw new Error(`OpenAPI access limit: (code:${res.data.code}) ` +
